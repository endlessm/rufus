...
static HANDLE GetHandle(char* Path, BOOL bLockDrive, BOOL bWriteAccess, BOOL bWriteShare)
{
	...
	if (bLockDrive) {
		...
		do {
			uprintf("Attempting FSCTL_LOCK_VOLUME for drive %s", Path);
...
// *ENDLESS* ENDLESS CARRIED A PATCH TO REPLACE THIS WITH LOCALIZATION STRING MSG_307
//but that appears to return stack local stuff, plus MSG_307 now has upstrea meaning.  pout.
	*label = STR_NO_LABEL;
